...
def nanskew(
    ...
    *,
    ...
    from the second and third central moment using a pairwise algorithm
    ...
    if axis is None:
        values_1d = values.ravel()
        mask_1d = None if mask is None else mask.ravel()
        result = skew_1d(values_1d, skipna=skipna, mask=mask_1d)
    elif values.ndim == 2 and (axis == 0 or axis == 1):
        result = skew_2d(values, axis=axis, skipna=skipna, mask=mask)
    elif values.ndim == 1 and axis == 0:
        result = skew_1d(values, skipna=skipna, mask=mask)
    else:
        raise ValueError(f"Invalid axis argument {axis!r}")
